    ...
    return (
        <div id={rowId} className={`result-row ${hasHashMatch ? 'is-target-row' : ''}`}>
            <div className="result-meta">
                ...
                {rowShortId &&
                    <a href={rowHref} className="row-anchor-id" title="Direct link to this row">#{rowShortId}</a>}
                {rowNick ? (
                    <button type="button" className="nick nick-button"
                            onClick={() => onNickClick(rowNick, result)}>{rowNick}</button>
                ) : (
                    <span className="nick"/>
                )}
                ...
                <span className="date"><a href={rowHref} className="row-anchor-time"
                                          title="Direct link to this row">{occurredAtText}</a></span>
                {result.permalink && (
                    <a href={getPermalinkUrl(result.permalink)} target="_blank" rel="noopener noreferrer"
                       className="permalink">
                        ↗
                    </a>
                )}
    ...
    return (
        <div id={rowId} className={`classic-row ${hasHashMatch ? 'is-target-row' : ''}`}>
            <div className="classic-main">
                {rowShortId && <a href={rowHref} className="row-anchor-id row-anchor-id-classic"
                                  title="Direct link to this row">#{rowShortId}</a>}
                ...
                {nick && (
                    <button type="button" className="classic-nick classic-nick-button"
                            onClick={() => onNickClick(nick, result)}>
                        {` <${nick}>`}
                    </button>
                )}
                ...
                {result.permalink && (
                    <a href={getPermalinkUrl(result.permalink)} target="_blank" rel="noopener noreferrer"
                       className="permalink permalink-inline">
                        ↗
                    </a>
                )}
            </div>
...
function getBulkSummaryHeader(result) {
    const raw = String(result.message_text ?? result.raw_line ?? '').trim();
    if (!raw) return '';
    if (!raw.includes('—')) {
        return raw;
...
function BulkEventCard({
                           result,
                           shareSearchQueryString,
                           includeSearchInAnchor,
                           onNickClick,
                           variant = 'refined',
                           showNetwork = true
                       }) {
    ...
    const summaryText = getBulkSummaryHeader(result);
        ...
        return (
            <div id={rowId} className={`classic-row classic-row--bulk ${hasHashMatch ? 'is-target-row' : ''}`}>
                <div className="classic-main classic-main--bulk">
                    <a href={rowHref} className="row-anchor-id row-anchor-id-classic"
                       title="Direct link to this row">#{rowShortId}</a>
    ...
    return (
        <div id={rowId} className={`result-row result-row--bulk ${hasHashMatch ? 'is-target-row' : ''}`}>
            <div className="result-meta bulk-meta">
                <span
                    className={`event-type ${`type-${eventType.toLowerCase().replace(/[^a-z0-9_-]/g, '-')}`}`}>{eventType}</span>
                {rowShortId &&
                    <a href={rowHref} className="row-anchor-id" title="Direct link to this row">#{rowShortId}</a>}
...
function StatsDailyChart({rows, chartType, showTotal, showChat, showEvents}) {
    ...
    if (chartType === 'pie') {
        ...
        return (
            <div className="stats-chart-wrap">
                <svg className="stats-chart" viewBox="0 0 560 320" role="img"
                     aria-label="Pie chart for selected statistics">
                    {pieValues.map((item) => {
                        const degrees = (item.value / total) * 360;
                        const start = cursor;
                        const end = cursor + degrees;
                        cursor = end;
                        return <path key={`pie-${item.key}`} d={pieSlicePath(180, 160, 120, start, end)}
                                     fill={item.color}/>;
                    })}
                </svg>
    ...
    return (
        <div className="stats-chart-block">
            <svg className="stats-chart" viewBox={`0 0 ${width} ${height}`} role="img"
                 aria-label={`${chartType} chart for daily activity`}>
                {yGridTicks.map((ratio) => {
                    const y = top + (plotHeight * ratio);
                    return (
                        <line key={`grid-y-${ratio}`} x1={left} y1={y} x2={left + plotWidth} y2={y} stroke="#1e293b"
                              strokeDasharray="3 4"/>
                    );
                })}
            ...
            <div className="stats-chart-date-range">Date
                range: {dateStartLabel || 'n/a'} → {dateEndLabel || 'n/a'}</div>
        </div>
    );
}
...
function StatsTopNicksChart({rows, dailyRows, selectedNicks, chartType}) {
            ...
            <div className="stats-chart-wrap">
                <svg className="stats-chart" viewBox="0 0 560 320" role="img" aria-label="Top-nicks pie chart">
                    {pieSlices.map((slice, idx) => {
                        ...
                        return <path key={`nick-pie-${nickRow.nick}`}
                                     d={pieSlicePath(180, 160, 120, slice.start, slice.end)}
                ...
                <div className="stats-chart-legend">
                    {pieSlices.map((slice, idx) => (
                        <div key={`nick-legend-${slice.row.nick}`} className="stats-chart-legend-row">
                            <span className="stats-color-dot"
                                  style={{backgroundColor: colorByNick[slice.row.nick] || palette[idx % palette.length]}}/>
    ...
    if (chartType === 'line') {
        ...
        return (
            <div className="stats-chart-block">
                <svg className="stats-chart" viewBox={`0 0 ${width} ${height}`} role="img"
                     aria-label="Top-nicks line chart over days">
                    {yGridTicks.map((ratio) => {
                        const y = top + (plotHeight * ratio);
                        return <line key={`nick-line-grid-${ratio}`} x1={left} y1={y} x2={left + plotWidth} y2={y}
                                     stroke="#1e293b" strokeDasharray="3 4"/>;
                    })}
                    <line x1={left} y1={top} x2={left} y2={yBottom} stroke="#334155"/>
                    <line x1={left} y1={yBottom} x2={left + plotWidth} y2={yBottom} stroke="#334155"/>
                    <text x={8} y={yForValue(maxValue) + 4} fill="#94a3b8"
                          fontSize="11">{compactNumber(maxValue)}</text>
                    <text x={16} y={yBottom + 4} fill="#94a3b8" fontSize="11">0</text>
                    ...
                    {xTickIndexes.map((idx) => {
                        ...
                        return (
                            <g key={`nick-line-tick-${idx}`}>
                                <line x1={x} y1={yBottom} x2={x} y2={yBottom + 5} stroke="#334155"/>
                                <text x={x} y={height - 10} fill="#94a3b8" fontSize="10"
                ...
                <div className="stats-chart-legend">
                    {selectedRows.map((row) => (
                        <div key={`line-legend-${row.nick}`} className="stats-chart-legend-row">
                            <span className="stats-color-dot"
                                  style={{backgroundColor: colorByNick[row.nick] || '#60a5fa'}}/>
                            <span>{row.nick}</span>
            ...
            {selectedRows.map((row, idx) => {
                ...
                return (
                    <g key={`nick-bar-${row.nick}`}>
                        <text x={left - 8} y={barY + (barH / 2) + 4} fill="#cbd5e1" fontSize="11"
                              textAnchor="end">{row.nick}</text>
                        <rect x={left + 1} y={barY} width={Math.max(barW, 1)} height={barH} fill="#60a5fa"
                              opacity="0.9"/>
                        <text x={left + barW + 6} y={barY + (barH / 2) + 4} fill="#94a3b8"
                              fontSize="11">{compactNumber(row.value)}</text>
                    </g>
                );
            })}
        </svg>
        ...
        return rows.map((row) => ({
            ...
            ...(function mapIdentity(rawRow) {
                const metadata = typeof rawRow?.metadata_json === 'string'
                    ? (() => {
                        try {
                            return JSON.parse(rawRow.metadata_json);
                        } catch {
                            return {};
                        }
                    })()
        ...
        <div className="page">
            ...
            <form onSubmit={handleSearch} className="search-form">
                ...
                <div className="form-row">
                    ...
                    {loadingNetworks &&
                        <small className="loading-hint"><span className="loading-spinner"/>Loading networks...</small>}
                </div>
                <div className="form-row">
                    ...
                    <input
                        ...
                        onBlur={(e) => {
                          const nextValue = String(normalizeChannelActivityThreshold(e.target.value));
                          setChannelActivityThreshold(nextValue);
                        }}
                        placeholder="30"
                    />
                ...
                <div className="form-row">
                    ...
                    {networkId && loadingChannels &&
                        <small className="loading-hint"><span className="loading-spinner"/>Loading channels...</small>}
                    {!loadingChannels && channels.length === 0 && <small>No channels found.</small>}
                    {!loadingChannels && channels.length > 0 && channelOptions.length === 0 &&
                        <small>No channels match your filter or activity threshold.</small>}
                    {!loadingChannels && channels.length > 0 && channelOptions.length > 0 && (
                        <small>{channelOptions.length} channels
                            available{networkId ? ' in selected network' : ' across all networks'}.</small>
                    )}
                                ...
                                <div className="nick-suggestion-list" role="listbox" aria-label="Nickname suggestions">
                                    {nickSuggestions.map((item) => (
                                        ...
                                        >
                                            ...
                                            {item.occurrences > 0 &&
                                                <span className="nick-suggestion-meta">{item.occurrences}</span>}
                                        </button>
                                    ))}
                                </div>
                            )}
                ...
                <div className="stats-panel">
                    <div className="stats-grid">
                        <div className="stats-card">
                            ...
                            <div className="stats-value">{Number(searchSummary.totalRows || 0).toLocaleString()}</div>
                        </div>
                        <div className="stats-card">
                            <div className="stats-label">Nicks</div>
                            <div className="stats-value">{Number(searchSummary.uniqueNicks || 0).toLocaleString()}</div>
                        </div>
                        <div className="stats-card">
                            <div className="stats-label">Dates</div>
                            <div className="stats-value">{Number(searchSummary.uniqueDates || 0).toLocaleString()}</div>
                        </div>
                        <div className="stats-card">
                            <div className="stats-label">Chat texts</div>
                            <div
                                className="stats-value">{Number(searchSummary.chatRowsTotal || 0).toLocaleString()}</div>
                        </div>
                        <div className="stats-card">
                            <div className="stats-label">Channel events</div>
                            <div
                                className="stats-value">{Number(searchSummary.channelEventRowsTotal || 0).toLocaleString()}</div>
                        </div>
                        <div className="stats-card">
            ...
            {results && (
                <div className="results">
                        ...
                        <div className="stats-panel">
                            <div className="stats-grid">
                                <div className="stats-card">
                                    ...
                                    <div
                                        className="stats-value">{Number(results.total_rows || 0).toLocaleString()}</div>
                                </div>
                                <div className="stats-card">
                                    <div className="stats-label">Unique dates</div>
                                    <div
                                        className="stats-value">{Number(results.unique_dates || 0).toLocaleString()}</div>
                                </div>
                                <div className="stats-card">
                                    <div className="stats-label">First seen</div>
                                    <div className="stats-value">{String(results.first_occurred_at || 'n/a')}</div>
                                </div>
                                <div className="stats-card">
                                    <div className="stats-label">Last seen</div>
                                    <div className="stats-value">{String(results.last_occurred_at || 'n/a')}</div>
                                </div>
                                <div className="stats-card">
                                    <div className="stats-label">Chat texts</div>
                                    <div
                                        className="stats-value">{Number(results.chat_rows_total || 0).toLocaleString()}</div>
                                </div>
                                <div className="stats-card">
                                    <div className="stats-label">Channel events</div>
                                    <div
                                    ...
                                    <div className="stats-section stats-chart-section">
                                        ...
                                        <div className="stats-chart-controls">
                                            ...
                                            <div className="stats-series-toggle">
                                                <label><input type="checkbox" checked={statsShowChat}
                                                              onChange={(e) => setStatsShowChat(Boolean(e.target.checked))}/> Chat
                                                    texts</label>
                                                <label><input type="checkbox" checked={statsShowEvents}
                                                              onChange={(e) => setStatsShowEvents(Boolean(e.target.checked))}/> Channel
                                                    events</label>
                                                <label><input type="checkbox" checked={statsShowTotal}
                                                              onChange={(e) => setStatsShowTotal(Boolean(e.target.checked))}/> Total</label>
                                            </div>
                                        </div>
                                        <StatsDailyChart
                                            rows={dailyRows}
                                ...
                                <div className="stats-chart-controls">
                                    ...
                                    <div className="stats-series-toggle">
                                        <button type="button" className="btn-secondary"
                                                onClick={() => setStatsSelectedTopNicks(availableTopNicks.slice(0, 8))}>
                                            Top 8
                                        </button>
                                        <button type="button" className="btn-secondary"
                                                onClick={() => setStatsSelectedTopNicks([...availableTopNicks])}>
                                            All
                                        </button>
                                    </div>
                                </div>
                            ...
                            {(() => {
                                ...
                                const renderPaginationControls = (positionKey = 'top') => (
                                    <div className={`results-pagination results-pagination--${positionKey}`}>
                                        <button type="button" className="btn-secondary" disabled={loading || !hasPrev}
                                        ...
                                        <div className="results-page-numbers" aria-label="Page numbers">
                                            {pageItems.map((item, idx) => {
                                                if (item === 'ellipsis') {
                                                    return <span key={`ellipsis-${positionKey}-${idx}`}
                                                                 className="results-page-ellipsis">…</span>;
                                                }
                  ...
                  Page {currentPage}{total > 0 ? ` / ${maxPageByTotal}` : ''}
                                        ...
                                        <button type="button" className="btn-secondary" disabled={loading || !hasNext}
                                                onClick={() => handlePageNavigation(currentPage + 1)}>
                                            Next
                                        </button>
                                    </div>
                            ...
                            <div className="results-header-row">
                                ...
                                <div className="results-controls">
                                    ...
                                    <div className="view-toggle">
                                        <button type="button" className={resultView === 'classic' ? 'active' : ''}
                                                onClick={() => setResultView('classic')}>Classic log view
                                        </button>
                                        <button type="button" className={resultView === 'refined' ? 'active' : ''}
                                                onClick={() => setResultView('refined')}>Refined cards
                                        </button>
                                    </div>
                                </div>
                            </div>
                            ...
                            {(() => {
                                ...
                                rows.forEach((row, idx) => {
                                    ...
                                    if (rowDate && rowDate !== previousDate) {
                                        rendered.push(
                                            <div key={`date-separator-${rowDate}-${idx}`}
                                                 className="result-date-separator">
                                                {rowDate}
                                            </div>
                                        );
                                    ...
                                    rendered.push(
                                        String(row.event_type || '').toUpperCase() === 'MODE_BULK'
                                            ? <BulkEventCard key={rowKey} result={row} variant={resultView}
                                                             showNetwork={!String(networkId || '').trim()}
                                                             shareSearchQueryString={lastSearchQueryString}
                                                             includeSearchInAnchor={includeQueryInAnchorLinks}
                                                             onNickClick={handleNickWhoisClick}/>
                                            : (resultView === 'classic'
                                                ? <ClassicResultRow key={rowKey} result={row}
                                                                    showNetwork={!String(networkId || '').trim()}
                                                                    shareSearchQueryString={lastSearchQueryString}
                                                                    includeSearchInAnchor={includeQueryInAnchorLinks}
                                                                    onNickClick={handleNickWhoisClick}/>
                                                : <RefinedResultRow key={rowKey} result={row}
                                                                    shareSearchQueryString={lastSearchQueryString}
                                                                    includeSearchInAnchor={includeQueryInAnchorLinks}
                                                                    onNickClick={handleNickWhoisClick}/>)
                                    );
                                ...
                                return (
                                    <div className="results-pagination results-pagination--bottom">
                                        ...
                                        <div className="results-page-numbers" aria-label="Page numbers">
                                            {pageItems.map((item, idx) => {
                                                if (item === 'ellipsis') {
                                                    return <span key={`ellipsis-bottom-${idx}`}
                                                                 className="results-page-ellipsis">…</span>;
                                                }
                                                const pageNumber = Number(item);
                                                const isActive = pageNumber === currentPage;
                ...
                <div className="whois-overlay" onClick={closeWhoisModal}>
                    <div className="whois-modal" onClick={(e) => e.stopPropagation()}>
                        ...
                        {!whoisState.loading && whoisState.error &&
                            <div className="whois-body whois-error">{whoisState.error}</div>}
                        {!whoisState.loading && !whoisState.error && (
                            <div className="whois-body">
                                ...
                                {whoisState.payload?.seen?.found && (
                                    <div className="whois-section">
                                        ...
                                        <div><strong>First
                                            seen:</strong> {String(whoisState.payload.seen.first_seen_at || 'n/a')}
                                        </div>
                                        <div><strong>Last
                                            seen:</strong> {String(whoisState.payload.seen.last_seen_at || 'n/a')}</div>
                                        <div><strong>Total
                                            rows:</strong> {Number(whoisState.payload.seen.total_rows || 0).toLocaleString()}
                                        </div>
                                        <div><strong>Active
                                            dates:</strong> {Number(whoisState.payload.seen.unique_dates || 0).toLocaleString()}
                                        </div>
                                        {whoisState.payload.seen.viewer_url && (
                                            <div>
                                                <a href={getPermalinkUrl(whoisState.payload.seen.viewer_url)}
                                                   target="_blank" rel="noopener noreferrer" className="permalink">
                                                    Open in viewer ↗
                                                </a>
                                            </div>
                                        )}
                                        {Array.isArray(whoisState.payload.seen.event_type_counts) && whoisState.payload.seen.event_type_counts.length > 0 && (
                                            <div className="whois-activity-list">
                                                {whoisState.payload.seen.event_type_counts.slice(0, 8).map((row) => (
                                                    <div key={`seen-type-${row.event_type}`}
                                                         className="whois-activity-row">
                                                        <span>{String(row.event_type || 'UNKNOWN')}</span>
                                                        <span>{Number(row.row_count || 0).toLocaleString()}</span>
                                                    </div>
                                                ))}
                                ...
                                {whoisState.payload?.whois?.latest_activity && (
                                    <div className="whois-section">
                                        ...
                                        <div>
                                            <strong>Time:</strong> {String(whoisState.payload.whois.latest_activity.occurred_at || 'n/a')}
                                        </div>
                                        <div>
                                            <strong>Type:</strong> {String(whoisState.payload.whois.latest_activity.event_type || 'n/a')}
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            <strong>Channel:</strong> {String(whoisState.payload.whois.latest_activity.channel_name || 'n/a')}
                                        </div>
                                    </div>
                                )}
                                {whoisState.payload?.whois?.whois && (
                                    <div className="whois-section">
                                        ...
                                        <div>
                                            <strong>Time:</strong> {String(whoisState.payload.whois.whois.occurred_at || 'n/a')}
                                        </div>
                                        <div>
                                            <strong>Type:</strong> {String(whoisState.payload.whois.whois.event_type || 'n/a')}
                                        </div>
                                        <div>
                                            <strong>to_nick:</strong> {String(whoisState.payload.whois.whois.to_nick || 'n/a')}
                                        </div>
                                        <div
                                            className="whois-raw-line">{String(whoisState.payload.whois.whois.raw_line || whoisState.payload.whois.whois.event_text || whoisState.payload.whois.whois.message_text || '')}</div>
                                    </div>
                                )}
                            </div>
                        )}
