...
### Changed
- Search network selector now depends on backend activity counts and no longer needs frontend-side guessing to hide empty networks.
...
- The channel activity threshold control is editable again without being hard-locked at 30, while still normalizing back to a minimum of 30 on commit; purge actions now present production-only deletion instead of sandbox/both scope choices.
